...
import {
  Plus, Trash2, Calendar, MapPin, Building2, Loader2, ArrowLeft, Info
} from 'lucide-react';
...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    ...
    const isIntrastate = !customer || !customer.state || customer.state.trim().toLowerCase() === 'maharashtra';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const loadToast = toast.loading('Saving invoice details...');
    try {
      ...
      toast.success('Invoice details updated successfully', { id: loadToast });
  ...
  if (loadingInv) {
    return (
      <div className="flex flex-col items-center justify-center py-40 text-muted-foreground">
        ...
        <p className="text-sm">Pre-loading invoice form...</p>
      </div>
    );
  }
  ...
  return (
    <>
      <Topbar
        ...
        subtitle="Modify buyer logistics, line items, and taxation details"
        action={{ label: 'Back to Details', onClick: () => router.push(`/invoices/${id}`), icon: ArrowLeft }}
      />
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent className="space-y-4 pt-2">
                ...
                <Button type="submit" className="w-full h-10 brand-gradient text-white mt-4 font-semibold">Save Invoice Modifications</Button>
              </CardContent>
            </Card>
